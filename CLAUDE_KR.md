...
- **샘플링은 `.greedy`.** 계획은 창작이 아니라 구조화된 명령이다. 같은 요청은 같은
...
- **`updateSlide`에서 빈 값은 "지우기"가 아니라 "건드리지 않기".** 도메인 모델은 둘을
  구분할 수 있지만 모델은 안정적으로 구분해 내지 못한다. 내용을 남기는 쪽이 안전하다.
- **세션은 요청 사이에 살아 있다.** 후속 요청("더 짧게")이 맥락을 가지도록. 컨텍스트
  초과 시 세션을 다시 만들고 한 번 재시도한다. 덱 개요는 매 프롬프트에 실려 가므로
  플래너가 실제로 필요로 하는 것은 잃지 않는다.
- **하지만 문서 사이에서는 살아남지 않는다.** 트랜스크립트는 슬라이드를 번호로
  가리키는데, `/close` 후 `/edit`을 하면 그 번호들은 더 이상 열려 있지 않은 덱을
  설명한다. 그래서 문서를 붙이거나 떼면 대화를 초기화한다 — 같은 이유로 둘 다 undo
  기록을 비운다. `/save-as`는 초기화하지 *않는다*. 파일과 문서 id는 바뀌지만 덱은
  그대로이기 때문이다. 두 경로 모두 `REPL.attachDocument` / `detachDocument`를
  거치므로, 나중에 추가될 문서 명령이 상태 초기화만 가져가고 대화를 남겨 두는 일이
  생기지 않는다.
- **액션마다 슬라이드 메타데이터를 다시 읽는다.** 각 액션이 다음 액션이 겨냥한 번호를
  밀어낸다.
- **실패한 단계에서 계획을 멈춘다.** 뒤 액션들은 앞 액션들이 만들어 낼 슬라이드
  번호를 전제하므로, 지나쳐 계속하면 엉뚱한 슬라이드를 편집한다. 적용된 것들은
  undo 스택에 남는다.
...
**테마 이름은 결국 이 단계의 문제가 아니었다.** 지역화되어 있는 것은 맞다 — 한국어
환경에서 `theme "White"`는 `-1728`로 실패하고 `theme "흰색"`이 성공한다 — 하지만
